...
using Recruitment_App.Services.Extentions;
...
app.MapControllerRoute(name: "default",
                pattern: "{controller=JobTitles}/{action=Index}/{id?}",
                defaults: new { controller = "JobTitles", action = "Index" });


app.MigrateDatabase<RecruitmentContext>((context, service) =>
{
    var logger = service.GetService<ILogger<RecruitmentContext>>();
    ContextSeeds.SeedAsync(context, logger).Wait();
});
